...
fp_df_cov <- file.path(h, "/aim_outputs/Aim2/C_frontier_analysis/", date_as, "/df_as_processed_rw_gbd.csv")
...
df_as_draws <- read_parquet(fp_as_draws)
...
##----------------------------------------------------------------
## 1. Table 1 NO UI - HIV & SUD & AUD & OUD JUST 2019
# 
# T1 - ALL State level table, HIV & SUD separate tables, states are ordered alphabetically
# 
...
# incidence rate
##----------------------------------------------------------------
acauses <- c("hiv", "mental_alcohol", "mental_drug_opioids", "_subs")

t1_2019_cols <- c("cause_name", "location_name", "spend_per_prev", "spend_all", 
...
t1_dol_cols <-  c("spend_per_prev", "spend_all", 
                  "spend_mdcd", "spend_mdcr", "spend_oop", "spend_priv")

for (a in acauses) {
  
  df_tmp <- df_as %>%
    dplyr::filter(acause == a) %>%
    dplyr::filter(year_id == 2019)
  ...
  # Select columns
  df_tmp <- df_tmp %>%
    select(all_of(t1_2019_cols))
  
  # Convert to dollars
  df_tmp <- convert_to_dollars(df_tmp, t1_dol_cols)
...
# Write to CSV
write.csv(df_t1_2019_hiv, file.path(dir_output, "T1_HIV_2019.csv"), row.names = FALSE)
write.csv(df_t1_2019_subs, file.path(dir_output, "T1_SUD_2019.csv"), row.names = FALSE)
write.csv(df_t1_2019_mental_alcohol, file.path(dir_output, "T1_AUD_2019.csv"), row.names = FALSE)
write.csv(df_t1_2019_mental_drug_opioids, file.path(dir_output, "T1_OUD_2019.csv"), row.names = FALSE)

##----------------------------------------------------------------
## 1. Table 1 UI - HIV & SUD & AUD & OUD JUST 2019
# 
# T1 - ALL State level table, HIV & SUD separate tables, states are ordered alphabetically
#
# Also adds "United States (National)" as the first row in the data.
# 
# Columns: State, Spending per prevalence (total spending for state / total summed prevalence count),
# Total spending (cumulative total state spending), 
# 4 total spending by payer (medicaid, medicare, private, oop), 
...
t1_2019_cols <- c("cause_name", "location_name", "draw", "spend_per_prev", "spend_all", 
                  "spend_mdcd", "spend_mdcr", "spend_oop", "spend_priv", "prevalence_counts", 
                  "as_mort_prev_ratio", "incidence_counts"
)
...
for (a in acauses) {
  ...
  df_tmp_draw_state_collapse <- df_tmp_draw_state_collapse %>%
    mutate(
      ...
      # Spend (no rounding)
      `Spending per prevalence` =
        paste0(spend_per_prev_mean, " (",
               spend_per_prev_lower, " - ",
               spend_per_prev_upper, ")"),
      
      `Total spending - All` =
        paste0(spend_all_mean, " (",
               spend_all_lower, " - ",
               spend_all_upper, ")"),
      
      `Total spending - Medicare` =
        paste0(spend_mdcr_mean, " (",
               spend_mdcr_lower, " - ",
               spend_mdcr_upper, ")"),
      
      `Total spending - Medicaid` =
        paste0(spend_mdcd_mean, " (",
               spend_mdcd_lower, " - ",
               spend_mdcd_upper, ")"),
      
      `Total spending - Out of pocket` =
        paste0(spend_oop_mean, " (",
               spend_oop_lower, " - ",
               spend_oop_upper, ")"),
      
      `Total spending - Private` =
        paste0(spend_priv_mean, " (",
               spend_priv_lower, " - ",
               spend_priv_upper, ")"),
      
      # Non-spend (round to 4 decimals)
      `Prevalence Count` =
        paste0(round(prevalence_counts_mean, 4), " (",
               round(prevalence_counts_lower, 4), " - ",
               round(prevalence_counts_upper, 4), ")"),
      
      `Mortality per case (AS)` =
        paste0(round(as_mort_prev_ratio_mean, 4), " (",
               round(as_mort_prev_ratio_lower, 4), " - ",
               round(as_mort_prev_ratio_upper, 4), ")"),
      
      `Incidence Count` =
        paste0(round(incidence_counts_mean, 4), " (",
               round(incidence_counts_lower, 4), " - ",
               round(incidence_counts_upper, 4), ")")
    ) %>%
    select(
      # Clean formatted columns first
      Cause, State,
      `Spending per prevalence`,
      `Total spending - All`,
      `Total spending - Medicare`,
      ...
      # Then ALL original columns for reference
      everything()
    )
...
write.csv(df_t2_mental_alcohol, file.path(dir_output, "T2_AUD.csv"), row.names = FALSE)
write.csv(df_t2_mental_drug_opioids, file.path(dir_output, "T2_OUD.csv"), row.names = FALSE)

##----------------------------------------------------------------
## 3. Table 3 - HIV & SUD 

#### DEPRECIATED NEW SE TABLE IS PRODUCED IN Decomp code 
# 
...
##----------------------------------------------------------------
## 4. Table 4 - HIV & SUD & AUD & OUD (one table by Year, one table by State each)
##
...
# ══════════════════════════════════════════════════════════════════
# 4e. AUD — State (no Ryan White)
# ══════════════════════════════════════════════════════════════════
df_t4_aud_state <- df_cov %>%
...
df_t4_aud_state <- convert_to_dollars(df_t4_aud_state,
                                      c("spend_mdcd", "spend_mdcr", "spend_oop", "spend_priv", "total_spend"))

df_t4_aud_state <- df_t4_aud_state %>%
  ungroup() %>%
  select(location_name, total_spend,
         spend_mdcr_pct, spend_mdcr,
         spend_mdcd_pct, spend_mdcd,
...
write.csv(df_t4_aud_state, file.path(dir_output, "T4a_AUD_state.csv"), row.names = FALSE)

# ══════════════════════════════════════════════════════════════════
# 4f. AUD — Year (no Ryan White)
# ══════════════════════════════════════════════════════════════════
df_t4_aud_year <- df_cov %>%
  filter(acause == "mental_alcohol") %>%
  select(cause_id, year_id, location_id, location_name,
         acause, cause_name,
         spend_mdcd, spend_mdcr, spend_oop, spend_priv) %>%
...
# Format YoY as strings
for (col in c("yoy_total", "yoy_mdcr", "yoy_mdcd", "yoy_oop", "yoy_priv")) {
  df_t4_aud_year[[col]] <- ifelse(is.na(df_t4_aud_year[[col]]), "",
                                  paste0(round(df_t4_aud_year[[col]], 2), "%"))
}

df_t4_aud_year <- convert_to_dollars(df_t4_aud_year,
                                     c("spend_mdcd", "spend_mdcr", "spend_oop", "spend_priv", "total_spend"))

df_t4_aud_year <- df_t4_aud_year %>%
  ungroup() %>%
  select(year_id, total_spend, yoy_total,
         spend_mdcr_pct, spend_mdcr, yoy_mdcr,
         spend_mdcd_pct, spend_mdcd, yoy_mdcd,
...
write.csv(df_t4_aud_year, file.path(dir_output, "T4b_AUD_year.csv"), row.names = FALSE)


# ══════════════════════════════════════════════════════════════════
# 4g. OUD — State (no Ryan White)
# ══════════════════════════════════════════════════════════════════
df_t4_oud_state <- df_cov %>%
...
df_t4_oud_state <- convert_to_dollars(df_t4_oud_state,
                                      c("spend_mdcd", "spend_mdcr", "spend_oop", "spend_priv", "total_spend"))

df_t4_oud_state <- df_t4_oud_state %>%
  ungroup() %>%
  select(location_name, total_spend,
         spend_mdcr_pct, spend_mdcr,
         spend_mdcd_pct, spend_mdcd,
...
write.csv(df_t4_oud_state, file.path(dir_output, "T4a_OUD_state.csv"), row.names = FALSE)

# ══════════════════════════════════════════════════════════════════
# 4h. OUD — Year (no Ryan White)
# ══════════════════════════════════════════════════════════════════
df_t4_oud_year <- df_cov %>%
  filter(acause == "mental_drug_opioids") %>%
  select(cause_id, year_id, location_id, location_name,
         acause, cause_name,
         spend_mdcd, spend_mdcr, spend_oop, spend_priv) %>%
...
# Format YoY as strings
for (col in c("yoy_total", "yoy_mdcr", "yoy_mdcd", "yoy_oop", "yoy_priv")) {
  df_t4_oud_year[[col]] <- ifelse(is.na(df_t4_oud_year[[col]]), "",
                                  paste0(round(df_t4_oud_year[[col]], 2), "%"))
}

df_t4_oud_year <- convert_to_dollars(df_t4_oud_year,
                                     c("spend_mdcd", "spend_mdcr", "spend_oop", "spend_priv", "total_spend"))

df_t4_oud_year <- df_t4_oud_year %>%
  ungroup() %>%
  select(year_id, total_spend, yoy_total,
         spend_mdcr_pct, spend_mdcr, yoy_mdcr,
         spend_mdcd_pct, spend_mdcd, yoy_mdcd,
...
write.csv(df_t4_oud_year, file.path(dir_output, "T4b_OUD_year.csv"), row.names = FALSE)


##----------------------------------------------------------------
## 5. Table 5 - HIV & SUD & AUD & OUD
#
...
df_t5_p_non_sud <- df_t5_p %>%
  filter(acause != "mental_drug_agg") %>% # filters down to hiv, alcohol, and opioids. groups by cause_name.
...
df_t5_p_final <- rbind(df_t5_p_non_sud, df_t5_p_sud)
...
write.csv(df_t5_p_final %>% filter(Cause == "Opioid use disorders"), file.path(dir_output, "T5_OUD.csv"), row.names = FALSE)
write.csv(df_t5_p_final %>% filter(Cause == "Alcohol use disorders"), file.path(dir_output, "T5_AUD.csv"), row.names = FALSE)
