"""
Filename: C_frontier_analysis_sfma_hiv_KxV_v4.py
Description: SFMA frontier analysis for HIV (Aim 2) — K × V composite outcome.

             Per the May 27, 2026 committee meeting (Joe Dieleman + Marcia
             Weaver): the primary cascade frontier outcome is K × V — the
             joint probability of being both diagnosed AND virally suppressed
             over total PWH.

             v4 CHANGES (May 28):
               1. ADDED lagged prevalence (t−1) as a covariate with POSITIVE
                  prior, motivated by the R verification regression results:
                     log_prev_per_100k_lag1: β = +0.37, p = 0.04 *
                  on logit(K × V). This adjusts the frontier for the
                  prevalence-induced achievable ceiling and isolates the
                  spending effect more cleanly.
               2. Lag is constructed FRESH inside the script (sort by
                  location_id × year_id, group_shift by 1). Does NOT trust
                  any pre-computed lag variable in the panel.
               3. Using t−1 (not contemporaneous) breaks the simultaneity
                  between current cascade and current prevalence (current
                  V reduces current onward transmission, which feeds back
                  into current prevalence — bad-control problem per
                  Cinelli, Forney & Pearl 2020). Lag is DAG-clean.
               4. NaN drop now also requires non-NaN lag — drops 2010
                  observations (one per state).
...
How to run: python C_frontier_analysis_sfma_hiv_KxV_v4.py
"""
# ═══════════════════════════════════════════════════════════════════════════════
...
dir_output = dir_base / today_yyyymmdd / "kxv_frontier_v4"
...
def run_sanity_checks(fp):
    print("\n" + "█" * 75)
    print("SANITY CHECKS — verifying inputs before fitting (v4: lagged prev)")
    print("█" * 75)
    ...
    print(f"      Year FE will use {years[0]} as reference, {len(years)-1} dummies.")
    print(f"      Lag spec drops year {years[0]} — effective panel: {years[1:]}.")

    print("\n" + "█" * 75)
    print(f"SANITY CHECKS PASSED — proceeding to SFMA fit (v4: lag prev as covariate)")
    print("█" * 75 + "\n")
...
def runSFA_HIV_KxV(df_full):
    ...
    print(f"\n{'=' * 75}")
    print(f"SFMA HIV K × V FRONTIER  (v4 — with lagged prev covariate)")
    ...
    ref_year = years[0]
    print(f"  Reference year (before lag drop): {ref_year}")
    for yr in years[1:]:
        col = f'year_{yr}'
    ...
    # ── Drop NaN on key vars (variance is optional; lag drops first year) ──
    ...
    # ────────────────────────────────────────────────────────────────────
    # v3/v4 CONVENTION: DO NOT NEGATE the outcome.
    ...
    # ── Standard error: variance column if present, else outcome SD fallback ──
    if variance_col in df.columns and df[variance_col].notna().any():
        se_val = float(np.sqrt(np.median(df[variance_col].dropna())))
        print(f"  Standard error: {se_val:.4f}  (from median of '{variance_col}')")
    else:
        se_val = float(df[outcome_col].std())
        print(f"  Standard error: {se_val:.4f}  (FALLBACK = SD of outcome, "
              f"'{variance_col}' missing or all-NaN)")
...
def plot_frontier(df_output, save_path=None):
    ...
    ax.set_title(
        f'HIV Cascade Efficiency Frontier — {OUTCOME_LABEL}\n'
        f'(n = {len(df_output)} state-years, log–logit space, v4 with lagged prev)',
        fontsize=13, fontweight='bold'
    )
    ax.legend(loc='lower right', fontsize=10)
    ax.grid(True, alpha=0.3, linestyle='--')

    cov_str = ('factor(year_id), race_prop_BLCK, race_prop_HISP, log_prop_homeless, '
               'log_prev_per_100k_lag1')
    fig.text(
        0.5, 0.01,
        f'Note: HIGHER logit(K × V) = more cascade coverage.   Frontier is the UPPER envelope:\n'
        f'best achievable K × V at each spending level. Points BELOW the frontier are inefficient.\n'
...
def plot_state_frontier(state_summary, save_path=None):
    ...
    ax.set_title(
        f'HIV Cascade Efficiency by State — {OUTCOME_LABEL}\n'
        f'(state means across 2011–2019, n = {len(plot_df)} states, v4 with lagged prev)',
        fontsize=13, fontweight='bold'
    )
    ax.grid(True, alpha=0.3, linestyle='--')
    ...
    fig.text(
        0.5, 0.01,
        f'Note: HIGHER logit(K × V) = better cascade coverage.   Green = more efficient, Red = less efficient.',
        ha='center', fontsize=8, style='italic', color='gray'
    )
...
if __name__ == '__main__':
    print("\n" + "=" * 75)
    print("SFMA FRONTIER ANALYSIS — HIV K × V (single pooled outcome)  v4")
    print("v4: added lagged prevalence (t-1) as a covariate with POSITIVE prior")
    print("=" * 75)
