...
##----------------------------------------------------------------
# Extract DEX population denominator before collapsing.
# pop does not vary by draw, toc, or acause — extract once per location/age/sex/year.
# Using payer == "all" as the representative row; pop is total population regardless of payer.
...
# Join DEX population denominator to df_m_pop
df_m_pop <- left_join(
  x = df_m_pop,
  y = df_dex_pop_lookup,
...
df_m_collapse <- df_m_pop %>%
  ...
  summarise(
    ...
    dex_pop = sum(dex_pop, na.rm = TRUE),
    .groups = "drop"
  )
...
df_m_collapse <- df_m_collapse %>%
  mutate(
    ...
    spend_per_capita  = (spend_all / dex_pop),
...
df_as <- df_as %>%
  ...
  summarise(
    ...
    as_spend_per_capita  = sum(spend_per_capita  * age_group_weight_value, na.rm = TRUE),
